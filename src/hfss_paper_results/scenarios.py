...
def condition_switch_labels(t: int, num_nodes: int) -> list[str]:
    labels = [rest_label(node) for node in range(num_nodes)]
    if 300 <= t < 600:
        for node in range(num_nodes):
            if node_type(node) == "surface":
