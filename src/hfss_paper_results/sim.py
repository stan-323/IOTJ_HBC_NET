...
from hfss_paper_results.data import HFSSLibrary, load_hfss_library
from hfss_paper_results.scenarios import (
    condition_name,
    condition_switch_labels,
    homogeneous_surface_labels,
    is_critical_node,
    node_type,
...
def _label_func(cfg: SimConfig, scenario: str) -> Callable[[int], list[str]]:
    ...
    return lambda t: condition_switch_labels(t, cfg.num_nodes)
...
def _allocate_energy(
    ...
    if method == "Proposed":
        ...
        priority += critical * (3.20 * urgency + 2.10 * short + 0.15)
...
def _allocate_data(
    ...
    if method == "ADT-MAC":
        ...
        weights *= np.where(critical, 0.55, 0.62)
    elif method == "w/o crit.-urg.":
...
def run_method(
    ...
    *,
    ...
    for t in range(cfg.T):
        ...
        decision_labels = homogeneous_surface_labels(t, cfg.num_nodes) if method == "w/o implant-aware" else true_labels
        decision_coeff = coefficients_for_labels(lib, cfg, local_wp, trace, decision_labels)
        ...
        if method in LP_METHODS:
            ...
            u_e = _allocate_energy(method, cfg, local_wp, decision_coeff, e, q, decision_critical, in_stress)
            u_d, mu_decision = _allocate_data(method, cfg, local_wp, decision_coeff, e, q, trace.rho, decision_critical, float(u_e.sum()))
...
def run_suite(
    ...
    *,
    ...
    lib = library or load_hfss_library()
    frames = [
        run_method(cfg, wp, method, seed, scenario, lib, budget_multiplier=budget_multiplier, power_multiplier=power_multiplier)
        for seed in seeds
