...
PROJECT_ROOT = Path(os.environ.get("IOTJ_WORKSPACE_ROOT", REPO_ROOT))
PACKAGE_ROOT = Path(os.environ.get("IOTJ_PACKAGE_ROOT", REPO_ROOT))
FINAL_DIR = Path(os.environ.get("IOTJ_FINAL_DIR", REPO_ROOT / "outputs" / "final"))
V1_DIR = Path(os.environ.get("IOTJ_STATE_ROBUSTNESS_DIR", REPO_ROOT / "data" / "state_robustness"))
OUT_DIR = Path(os.environ.get("IOTJ_ADAPTIVE_OUTPUT_DIR", REPO_ROOT / "outputs" / "state_robustness_iter10_adaptive"))
...
from hfss_paper_results.config import SimConfig, WorkPoint  # noqa: E402
from hfss_paper_results.data import HFSSLibrary, load_hfss_library  # noqa: E402
from hfss_paper_results.metrics import summarize_raw  # noqa: E402
from hfss_paper_results.scenarios import condition_name, node_type  # noqa: E402
from hfss_paper_results.sim import _label_func, coefficients_for_labels, generate_trace, solve_slot_lp  # noqa: E402
...
def run_adaptive_iterations() -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    failed = {key: value for key, value in sanity.items() if value > 1e-9}
    if failed:
        raise RuntimeError(f"Iteration 1 p=0 sanity check failed: {failed}")
    sanity_text = ["p=0 sanity check against final Fig. 3: PASS"]
...
def plot_comprehensive_figS2(leaderboard: pd.DataFrame) -> None:
    ...
    v1_summary_path = local_v1_summary if local_v1_summary.exists() else V1_DIR / "state_misclassification_summary.csv"
    v1 = pd.read_csv(v1_summary_path).sort_values("p_err")
...
def write_random_baseline_snippet() -> None:
    summary = pd.read_csv(V1_DIR / "state_misclassification_summary.csv")
