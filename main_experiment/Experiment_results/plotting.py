...
from Experiment_results.config import ABLATION_METHODS, MAIN_METHODS, METHOD_COLORS, METHOD_LINESTYLES, STRESS_METHODS, SimConfig
...
def apply_ieee_scheme_a_style(style_path: Path | None = None) -> None:
    if style_path is None:
        style_path = Path(__file__).resolve().parents[1] / "ieee_trans.mplstyle"
...
def _write_redesign_manifest(out_dir: Path, source_dir: Path, style_path: Path, timestamp: str) -> None:
    ...
    lines = [
        ...
        "- original `output` files were not modified by this redesign mode",
        "",
        "## Palette",
        *palette_lines,
    ]
...
def redesign_final_figures(source_dir: Path, *, output_root: Path | None = None, timestamp: str | None = None, style_path: Path | None = None) -> Path:
    ...
    style_path = style_path or Path(__file__).resolve().parents[1] / "ieee_trans.mplstyle"
