...
def _phase_counts(phase: pd.DataFrame) -> tuple[float, float, float]:
    ...
    medium = float(((vals >= 0.03) & (vals <= 0.50)).sum() / total)
    high = float((vals > 0.50).sum() / total)
...
def evaluate_candidate_gates(metrics: dict[str, dict[str, float]], phase: pd.DataFrame | None) -> GateReport:
    ...
    _add(rows, failed, "G4", _m(metrics, "Oracle", "stress_shortage_severity") <= 0.05, "stress oracle near-feasible sanity check")
    _add(
        rows,
        failed,
        "G5",
    ...
    _add(
        ...
        failed,
        "F4-G3a",
        _m(metrics, "Proposed", "em_util_p95") >= 0.60,
        "EM budget is binding under stress: em_util_p95 >= 0.60",
    )
    _add(
        rows,
        failed,
        "F4-G3b",
        _m(metrics, "Proposed", "em_util_mean") <= 0.85,
        "EM headroom preserved: em_util_mean <= 0.85",
    )
    ...
    _add(
        ...
        failed,
        "F6-G3",
        no_impl_short >= p_short + 0.02
        or _m(metrics, "w/o implant-aware", "implant_energy_p05") <= _m(metrics, "Proposed", "implant_energy_p05") - 0.01
        or _m(metrics, "w/o implant-aware", "recovery_time") >= _m(metrics, "Proposed", "recovery_time") + 1.0
        or _m(metrics, "w/o implant-aware", "em_util_mean") <= _m(metrics, "Proposed", "em_util_mean") - 0.05
        or _m(metrics, "w/o implant-aware", "surface_backlog_mean") >= _m(metrics, "Proposed", "surface_backlog_mean") + 0.01,
        "coarse rest/stress library shows measurable degradation",
    )
