...
def paired_wilcoxon_pvalue(proposed_per_seed: pd.Series, baseline_per_seed: pd.Series, *, alternative: str = "two-sided") -> float:
    proposed = pd.Series(proposed_per_seed, dtype=float).reset_index(drop=True)
    baseline = pd.Series(baseline_per_seed, dtype=float).reset_index(drop=True)
    size = min(len(proposed), len(baseline))
    if size == 0:
        return float("nan")
    diff = proposed.iloc[:size] - baseline.iloc[:size]
    if int((diff.abs() > 1e-12).sum()) == 0:
        return 1.0
    try:
        return float(wilcoxon(proposed.iloc[:size], baseline.iloc[:size], zero_method="wilcox", alternative=alternative, method="auto").pvalue)
    except ValueError:
        return 1.0
...
def summarize_raw(raw: pd.DataFrame, cfg: SimConfig, *, scenario: str) -> MetricSummary:
    raw = raw.copy()
    if "solve_time_ms" not in raw.columns:
        raw["solve_time_ms"] = 0.0
    ...
    for (seed, method), group in raw.groupby(["seed", "method"], sort=False):
        ...
        slots = group.groupby("t", sort=False).agg(
            ...
            solve_time_ms=("solve_time_ms", "first"),
        )
        ...
        row = {
            ...
            "solve_time_ms_mean": float(slots["solve_time_ms"].mean()) if not slots.empty else 0.0,
            "solve_time_ms_p95": float(slots["solve_time_ms"].quantile(0.95)) if not slots.empty else 0.0,
            "solve_time_ms_max": float(slots["solve_time_ms"].max()) if not slots.empty else 0.0,
        }
